...
CACHE_FILE = "../../../data/01_intermediate/phase2_assembly/cache_pubchem_scores.csv"
...
def main():
    ...
    cache_dict = {r['Drug_Name']: r for r in cached_rows}
    print(f"Loaded {len(cache_dict)} entries from Cautious Cache.")

    results = []

    for idx in tqdm(range(len(df)), total=len(df), desc="Xử lý"):
        row = df.iloc[idx]
        name, smiles, pid = row.get("Drug_Name", ""), row.get("SMILES", ""), row.get("PubChem_ID", "")

        if name in cache_dict:
            results.append(cache_dict[name])
            continue
        ...
        if not cid:
            res_row = {"Drug_Name": name, "SMILES": smiles, "PubChem_ID": pid,
                            "CID": None, "CID_Source": "none", "score": 0,
                            "status": "no_cid", "PUG_View": 0, "Assay_Summary": 0}
            results.append(res_row)
            cache_dict[name] = res_row
            time.sleep(SLEEP)
            if len(results) % CP_EVERY == 0:
